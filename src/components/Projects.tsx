...
const Projects: React.FC = () => {
  const projectData = [
    ...
    },
    {
      ...
      description:
        "This React project mimics YouTube, fetching video data through the YouTube API. It employs components for the header, left navigation, and video details. Global state management is facilitated by the React Context API, ensuring dynamic interaction and rendering of video-related details.",
  ...
  return (
    <section
      id="section--3"
      className="relative flex flex-col items-center px-6 py-16 pb-16 mb-5 ml-2 mr-2 font-sans text-sm font-light bg-black md:mb-14 md:ml-20 md:mr-20 rounded-tl-3xl rounded-tr-3xl rounded-xl gap-9 md:pb-24 lg:gap-14 lg:pb-32"
    >
      <div
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        {projectData.map((project, index) => (
          <div
            key={index}
            className="flex flex-col justify-between max-w-sm px-3 py-4 mx-auto mb-10 duration-100 bg-white border border-gray-300 rounded-lg hover:shadow-lg"
          >
            <div>
              <div>
                <img
            ...
            <div>
              <div className="flex flex-wrap gap-2 my-4">
                {project.techStack.map((tech, index) => (
                  <div
                    key={index}
                    className="px-2 py-1 text-xs text-black rounded-md bg-zinc-200"
                  >
                    {tech}
                  </div>
                ))}
